...
public class CameraOverlayView extends View implements SensorEventListener, LocationListener {
    ...
    private void drawMyPoint(Location location1, PointDTO point, Canvas canvas) {
        ...
        float curBearingToMW = location1.bearingTo(pointLocation);
        ...
        if (gotRotation) {
            ...
            if (gotRotation) {
                ...
                int pointColor = extractPointColor(point);
...
//                Log.d("ANGLE max h", String.valueOf(canvas.getHeight()));
                ...
                if ((location1.distanceTo(pointLocation) < 150000 * 0.3) && (location1.distanceTo(pointLocation) > 80000 * 0.3)) {
                    circleRadius -= (10 * location1.distanceTo(pointLocation) / (150000 * 0.3));
                } else if ((location1.distanceTo(pointLocation) <= 80000 * 0.3)) {
                    circleRadius = 20f;
                } else {
                    circleRadius = 10f;
                }
    ...
    private int extractPointColor(PointDTO point) {
        double altitude = point.getAltitude();
        double altitudePercent = altitude / 35000D;

        altitudePercent = altitudePercent > 1D ? 1D : altitudePercent;
